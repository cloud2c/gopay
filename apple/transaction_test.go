...
func TestGetTransactionHistoryV2(t *testing.T) {
	// Query Parameters https://developer.apple.com/documentation/appstoreserverapi/get-v2-history-_transactionid_
	bm := gopay.BodyMap{
		"sort": "ASCENDING",
	}
	originalTransactionId := "2000000184445477"
	rsp, err := client.GetTransactionHistoryV2(ctx, originalTransactionId, bm)
	if err != nil {
		if statusErr, ok := IsStatusCodeError(err); ok {
			xlog.Errorf("%+v", statusErr)
			// do something
