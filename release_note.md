...
* 修改记录：
  * 支付宝v3：**不兼容变更** —— NewClientV3 默认要求验签（verifyRequired 由 false 改为 true）。既没调 SetCert 也没调 SetAliPayPublicKey 的 client，第一次调用即报错，不再静默放过一个未经验证的响应。
    * 旧默认值是为了不打断调用方，但它让故障模式变得不可见：v3 里只有这两个方法会设置支付宝公钥，两个都没调时每个响应都验签“通过”且 err 为 nil。支付 SDK 悄悄停止验签，方向错了。
    * 正确配置了证书或公钥的调用方不受影响。确实接受不验签的场景（临时排障、尚未配密钥的沙箱应用）显式调 client.SetVerifyRequired(false) 退出。
    * 错误信息同时列出两个入口（此前只提 SetCert），并指明退出方式。
